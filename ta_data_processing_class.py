...
import datetime
#import numpy as np
from numpy import array, linspace, interp, isfinite, std, divide
...
class ta_data_processing:
    ...
    def __init__(self,probe_array,reference_array,first_pixel,num_pixels):
        ...
        #self.raw_probe_array = self.probe_array
        #self.raw_reference_array = self.reference_array
    ...
    def set_linear_pixel_correlation(self):
        ...
        pr_corr[::2] = pr_corr[::2]/pr_corr[1::2] #mean a / mean b
        pr_corr[1::2] = pr_corr[1::2]/pr_corr[1::2] #this equals 1?? mean b/mean b
        #pr_corr[1::2] = 1
    ...
    def linear_pixel_correlation(self,linear_corr):
        self.probe_array = self.probe_array/linear_corr[0] #a / (mean a / mean b) and b stays same
        #self.probe_array[:,::2]= self.probe_array[:,::2] *self.probe_array.mean(axis=0)[1::2]/self.probe_array.mean(axis=0)[::2]
        self.reference_array = self.reference_array/linear_corr[1]
        return

    #def linear_pixel_correlation_new(self,linear_corr):
    #    self.probe_array = np.divide(self.probe_array,linear_corr[0])
    #    self.reference_array = np.divide(self.reference_array,linear_corr[1])
    #    return
    ...
    #basically, if statements are slow because python guesses which one to do, then if it guesses wrong it does the Other
    #what I am doing is the same thing, but my guess is educated... i think
    #so this will speed things up when Tau is false, but slow them down when it is true...
    def separate_on_off_new(self, tau_flip_request = True):
        self.probe_on_array = self.probe_array[::2,:]
        self.probe_off_array = self.probe_array[1::2,:]
        self.reference_on_array = self.reference_array[::2,:]
        self.reference_off_array = self.reference_array[1::2,:]
        if tau_flip_request is True:
            self.probe_on_array, self.probe_off_array = self.probe_off_array, self.probe_on_array
            self.reference_on_array, self.reference_off_array =self.reference_off_array, self.reference_on_array
    ...
    def manipulate_reference(self,refman):
        ...
        x = #linspace(0,self.num_pixels-1,self.num_pixels)
        new_x = ((x-sc)*sf)+sc-ho
        for i,spectra in enumerate(self.reference_off_array):
            self.reference_off_array[i] = np.interp(new_x,x,spectra*vs+vo)
        #self.reference_off_array = [interp(new_x,x,spectra*vs+vo) for spectra in self.reference_off_array]
        for i,spectra in enumerate(self.reference_on_array):
            self.reference_on_array[i] = np.interp(new_x,x,spectra*vs+vo)
        #self.reference_on_array = [interp(new_x,x,spectra*vs+vo) for spectra in self.reference_on_array]
        return
    ...
    def calculate_dtt(self,use_reference=False,cutoff=[0,100],use_avg_off_shots=True,max_dtt=1):
        ...
        #fin_dtt = self.dtt[isfinite(self.dtt)]
