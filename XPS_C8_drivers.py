...
class XPS:
	...
	def __init__(self):
         self.MAX_NB_SOCKETS = 100
         self.nbSockets = 0
         self.sockets = {}
         self.usedSockets = {}
         for socketId in range(self.MAX_NB_SOCKETS):
             self.usedSockets[socketId] = 0

	# Send command and get return
	def __sendAndReceive(self, socketId, command):
		try:
			self.sockets[socketId].send(command.encode('utf-8'))
			ret = self.sockets[socketId].recv(1024).decode('utf-8')
			while (ret.find(',EndOfAPI') == -1):
				ret += self.sockets[socketId].recv(1024)
	...
	def TCP_ConnectToServer(self, IP, port, timeOut):
         socketId = 0
         if (self.nbSockets < self.MAX_NB_SOCKETS):
             while (self.usedSockets[socketId] == 1 and socketId < self.MAX_NB_SOCKETS):
                 socketId += 1
         ...
         self.usedSockets[socketId] = 1
         self.nbSockets += 1
         try:
             self.sockets[socketId] = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
             self.sockets[socketId].connect((IP, port))
             self.sockets[socketId].settimeout(timeOut)
             self.sockets[socketId].setblocking(1)
         except socket.error:
             return -1
	...
	def GroupHomeSearch (self, socketId, GroupName):
		if (self.usedSockets[socketId] == 0):
			return
