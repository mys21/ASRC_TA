...
class octoplus(QObject):
    ...
    def Initialize(self,lines_per_frame=45000):
        ...
        InitializeLibrary.restype = None
        InitializeLibrary()

        UpdateCameraList(byref(self.ulNbCameras))
 
        GetCameraInfo.restype = POINTER(tCameraInfo)
        GetCameraInfo(ulIndex,byref(CameraInfo)) 

        OpenCamera.restype = c_void_p
        OpenCamera(byref(CameraInfo), byref(hCamera))

        WriteRegister.restype = c_size_t
        ulAddress= c_ulong(0x4F000000) 
        ulValue = c_ulong(1)  #contextual data on or off
        iSize = c_size_t(ulValue.__sizeof__())
        WriteRegister = WriteRegister(hCamera, ulAddress, byref(ulValue), byref(iSize))

        ulAddress= c_ulong(0x4F000018) 
        ulValue = c_ulong(0)  #circular buffer on or off
        WriteRegister(hCamera, ulAddress, byref(ulValue), byref(iSize))
