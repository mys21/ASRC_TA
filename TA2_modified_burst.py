...
from TA2_camera_modified_burst import octoplus
from ta_data_processing_class_modified_burst import ta_data_processing
...
#        self.disco_times = np.linspace(min(self.times),max(self.times),((max(self.times)-min(self.times))/self.bin_length)+1)
    ...
    def post_acquire(self,probe,first_pixel,num_pixels):
        '''process ta data according to functions found in ta_data_processing_modified_burst.py'''
        try:
